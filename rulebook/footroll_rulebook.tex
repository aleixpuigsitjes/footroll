...
Actions are resolved by dice rolls and formulas. Following a generic interpretation of these formulas is provided.
...
where \emph{k} is a scaling factor that varies depending on the action.
...
\begin{note}
\begin{itemize}
\item Place the goalkeeper at the last row of the field in front of the goal line.
\item This sequence applies to all restarts, including \emph{kick-off}, after an \emph{out }(\emph{throw-in}, \emph{corner-kick, goal-kick}) or after a \emph{foul} (\emph{free-kick}).
\end{itemize}
\end{note}
...
\begin{note}
\begin{itemize}
\item The action resolution phase does not imply the passage of time and all actions are considered to occur simultaneously.
\item An action resolved during the action resolution phase may imply the movement of the ball. The ball, however, does not move during the action resolution phase; but during the movement phase, simultaneously with the footballers.
\end{itemize}
\end{note}
...
\begin{note}
\begin{itemize}
\item The receiver of a pass cannot move with possession of the ball in the same turn when the ball is received, as the movement of the ball and footballers occur simultaneously and the ball cannot move further.
\item A defender only blocks an attacker during the action resolution phase (see \emph{blocking} section). The rational for this is that the movement of the attacker and the defender occurs simultaneously, but it is resolved sequentially. A defender may prefer to retreat before being forced to tackle.
\end{itemize}
\end{note}
...
\begin{hint}
\begin{itemize}
\item Be aware that blocking is an action that requires dice rolls. A failure can expose your defense, and there is the possibility of committing a foul. Block only when necessary.
\item Place the defender one yard away from the attacker's position for a "vigilant" non-blocking defensive position.
\end{itemize}
\end{hint}
...
\begin{note}
\begin{itemize}
\item A forward pass is possible, where the ball's movement anticipates the receiver's movement. An attacker can make a forward pass to themselves.
\item If a pass receiver has a defender in their \emph{reach zone} when they reach the ball, a \emph{challenged ball }must be resolved to determine who takes control (see \emph{challenged ball }section).
\end{itemize}
\end{note}
...
\item A \emph{low-pass} is automatically successfully resolved if the distance is shorter than the maximum distance allowed by the \emph{low-pass} skill.
\item A \emph{low-pass} longer than the maximum distance has to be resolved with a die roll using the \emph{low-pass} skill.
\end{itemize}
\end{rulebox}
...
\begin{note}
\begin{itemize}
\item In Footroll, due to its turn-based nature, defenders move after attackers in the Movement Phase, enabling them to systematically advance their defensive line in subsequent turns and force attackers into offside positions before the Action Resolution Phase.
\item The attacker \textbf{cannot} perform a \emph{one-touch} action in this case, like in forward passes.
\end{itemize}
\end{note}
...
\begin{hint}
To perform an \emph{orthogonal move} (x, y) in a grid, swap x and y components and invert the direction of one of the two components. Two possible directions can be freely chosen: (-y, x) or (y, -x).
\end{hint}
...
\begin{note}
\begin{itemize}
\item Note that 20 = 100/5, 25 = 100/4, 33 = 100/3 and 50 = 100/2.
\item Placing fewer wall defenders than the \emph{maximum effective wall defenders} reduces the effectiveness of the defensive wall.
\item If there are more wall defenders than the \emph{maximum} \emph{number of effective wall defenders}, the excess of wall defenders is useless, unless the shot has an \emph{effect} or in case of an indirect kick.
\item If the attacker places footballers at the wall position, they similarly count as defenders at the wall.
\end{itemize}
\end{note}
...
\begin{xltabular}{\textwidth}{>{\raggedright\arraybackslash}p{0.15\textwidth}>{\raggedright\arraybackslash}p{0.25\textwidth}>{\raggedright\arraybackslash}p{0.25\textwidth}>{\raggedright\arraybackslash}X}
\toprule
...
\bottomrule
\endlastfoot
\textbf{Dribble} & $D = A_{\text{tackle/hand-tackle}}$ \newline \textbf{Hand-tackle:} $S + 20$ & $A > D \rightarrow$ success \newline $A = D \rightarrow$ draw \newline $A < D \rightarrow$ failure & Swap, defender's $V/2$ \newline Repeat \newline Swap, attacker's $V/2$ \\
\midrule
\textbf{Feint} & $D = A_{\text{tackle/hand-tackle}}$ \newline \textbf{Hand-tackle:} $S + 20$ & $A > D \rightarrow$ success \newline $A = D \rightarrow$ draw \newline $A < D \rightarrow$ failure & Swap, defender's $V/2$ \newline Repeat \newline Swap, attacker's $V/2$ \\
\midrule
\textbf{Tackle} & $D = A_{\text{feint}}$ & $A > D \rightarrow$ success \newline $A = D \rightarrow$ draw \newline $A < D \rightarrow$ failure & Swap, defender's $V/2$ \newline Repeat \newline Swap, attacker's $V/2$ \\
\midrule
\textbf{Slip} & $D = A_{\text{mark}}$ & $A > D \rightarrow$ success \newline $A = D \rightarrow$ draw \newline $A < D \rightarrow$ failure & Swap, defender's $V/2$ \newline Repeat \newline Swap, attacker's $V/2$ \\
\midrule
\textbf{Intercept} & $D = A_{\text{shot/pass}}$ & $A \geq D \rightarrow$ success \newline $A < D \rightarrow$ failure & See shot or pass \\
\midrule
\textbf{Low-pass ($\leq$ 30)}\textsuperscript{(1)} & $D = L$ & \textbf{Success distance:} \newline $S_{\text{low-pass}} = 50 \rightarrow 5$ y \newline $S_{\text{low-pass}} = 60 \rightarrow 10$ y \newline $S_{\text{low-pass}} = 70 \rightarrow 15$ y \newline $S_{\text{low-pass}} = 80 \rightarrow 20$ y \newline $S_{\text{low-pass}} = 90 \rightarrow 25$ y \newline $S_{\text{low-pass}} = 100 \rightarrow 30$ y \newline \textbf{Otherwise:} \newline $A \geq D \rightarrow$ success \newline $A < D \rightarrow$ error \newline $E = (D - A)/2$ & Error direction $\rightarrow$ D8 \\
\midrule
\textbf{High-pass (> 30)} \newline \textbf{Goal-kick} & $D = L$ & $A \geq D \rightarrow$ success \newline $A < D \rightarrow$ error \newline $E = (D - A)/2$ & Error direction $\rightarrow$ D8 \\
\midrule
\textbf{Throw-in} \newline \textbf{Hand-pass} & $D = 2 \cdot L$ & $A \geq D \rightarrow$ success \newline $A < D \rightarrow$ error \newline $E = (D - A)/2$ & Error direction $\rightarrow$ D8 \\
\midrule
\textbf{Shot} \newline \textbf{Penalty} \newline \textbf{Finish }\textsuperscript{(1)(L)} \newline \textbf{Scissors-kick }\textsuperscript{(1)(H)} \newline \textbf{Offensive header }\textsuperscript{(1)(H)} & $D = D_{\text{shot}}$ \newline \textbf{Scissors-kick:} \newline $D = D_{\text{shot}} + 20$ \newline \textbf{Offensive header:} \newline $D = 2 \cdot D_{\text{shot}}$ & \textbf{Shot placement:} \newline $A > D \rightarrow$ GK reach? \newline $A = D \rightarrow$ Post \newline $A < D \rightarrow$ Out \newline \textbf{Target displacement:} \newline $R = A-D$ \newline \textbf{Shot/header power:} \newline $R = A/100 \cdot (100 - D)$ & \textbf{Displacement:} $R/5$ cells from \emph{closest} target \newline \RulebookInlineImage[0.12\textwidth]{images/image16.png} \newline \textbf{Post:} Direction $\rightarrow$ D8 \newline D8=[1,3]: back to field \newline D8=[4,5]: outside of field \newline D8=[6,8]: towards goal \newline \RulebookInlineImage[0.12\textwidth]{images/image8.png} \\
\midrule
\textbf{Free-kick} & $D = D_{\text{shot}} + D_{\text{wall}}$ \newline $D_{\text{wall}} = 20 \cdot \frac{\min(N_{\text{wall}}, N_{\text{eff}})}{N_{\text{eff}}}$ \newline $N_{\text{wall}}$ \textbf{:} wall defenders \newline $N_{\text{eff}}$ \textbf{:} effective wall defenders \newline $D_{\text{shot}} = [01, 20] \rightarrow N_{\text{eff}} = 5$ \newline $D_{\text{shot}} = [21, 25] \rightarrow N_{\text{eff}} = 4$ \newline $D_{\text{shot}} = [26, 33] \rightarrow N_{\text{eff}} = 3$ \newline $D_{\text{shot}} = [34, 50] \rightarrow N_{\text{eff}} = 2$ \newline $D_{\text{shot}} = [51, 100] \rightarrow N_{\text{eff}} = 1$ & \textbf{Shot placement:} \newline $A > D \rightarrow$ GK reach? \newline $A = D \rightarrow$ Post \newline $A < D$ and: \newline  $A \geq D_{\text{shot}} \rightarrow$ Wall \newline  $A < D_{\text{shot}} \rightarrow$ Out \newline \textbf{Target displacement:} \newline $R = A-D$ \newline \textbf{Shot power:} \newline $R = A/100 \cdot (100 - D)$ & See shot \newline A hit to the wall is resolved as a failed \emph{pass} \\
\midrule
\textbf{Effect} & $D = L/2$ & \textbf{Displacement:} \newline $R_X = A/100 \cdot L_X/2$ \newline $R_Y = A/100 \cdot L_Y/2$ & \textbf{Original position:} \newline  $(O_x, O_y)$ \newline \textbf{Virtual positions:} \newline $(O_x-R_y, O_y+R_x)$ or \newline $(O_x+R_y, O_y-R_x)$ \\
\midrule
\textbf{Reach} & \textbf{GK:} $D = 10 \cdot L$ \newline \textbf{Defender:} $D = 33 \cdot L$ \newline $L =$ distance from GK to target & $A \geq D \rightarrow$ GK catch? \newline $A < D \rightarrow$ goal & \RulebookInlineImage[0.12\textwidth]{images/image7.png} \\
\midrule
\textbf{Catch} & $D = 2 \cdot \text{shot/header power}$ &  $A \geq 2 \cdot D \rightarrow$ catch \newline $D \leq A < 2 \cdot D \rightarrow$ corner \newline  $A < D \rightarrow$ goal &  \\
\end{xltabular}
\endgroup

\clearpage
\section{Shot difficulty values calculation}

\subsection{Overview}

This document explains, for information purposes only, the formula used to pre-compute the \textbf{difficulty} of a shot encoded in the grid values. It is based on:

\begin{itemize}
    \item the Euclidean distance to the target,
    \item the angular aperture available,
\end{itemize}

The computation proceeds in four steps: distance, aperture, normalization, and combination.

\subsection{Distance}

Given horizontal and vertical offsets $(dx, dy)$, the raw Euclidean distance is:

\[
d = \sqrt{dx^{2} + dy^{2}}
\]

This distance is normalized by the maximum possible distance on the field:
...
\frac{
    ...
    \sqrt{\left(\frac{W_f}{2}\right)^{2} + \left(\frac{H_f}{2}\right)^{2}}
}
\]

Here, $W_f$ and $H_f$ represent the maximum field width and height in cells.

\subsection{Aperture}

The \textbf{aperture} models how wide the goal appears from the shooter's perspective.  
If $W_g$ is the goal width, the aperture is computed using two arctangents:

\[
a =
\begin{cases}
\arctan\!\left(\dfrac{dy + \frac{W_g}{2}}{dx}\right)
-
\arctan\!\left(\dfrac{dy - \frac{W_g}{2}}{dx}\right),
& dx \neq 0 \\[10pt]
0, & dx = 0.
...
The aperture is normalized to the range $[0, 1]$ by dividing by $\pi$:

\[
a_{\text{norm}} = \frac{a}{\pi}
\]

\subsection{Difficulty Components}

Two difficulty components are compared:

\[
D_1 = d_{\text{norm}}, \qquad
D_2 = (1 - a_{\text{norm}})^{15}
\]

\begin{itemize}
    \item $D_1$ increases with distance.
    \item $D_2$ increases sharply when the aperture becomes small, because the exponent $15$ creates a strong non-linearity. This value is experimental.
\end{itemize}

\subsection{Final Difficulty}

The difficulty is the maximum of the two components, scaled to $[0, 100]$:

\[
D = 100 \cdot \max(D_1,\, D_2)
\]

A final clipping ensures the difficulty stays within bounds:
...
This value is the final difficulty score.

\begin{figure}[H]
\centering
\includegraphics[width=1.0\textwidth]{images/shot_difficulty_heat_map.png}
\caption{Shot difficulty heat map.}
\end{figure}
